...
#define DEFAULT_CSS ":/resources/standart.css"
...
void ThemeLoader::loadTheme(QString& themeName)
{
    bool loading = true;
    while (loading) {
        int ret = this->loadLayout(themeName, ":/resources/");
...
void ThemeLoader::findColorStyles(QMenu *colors, const QString& configSelectedStyle)
{
    ...
    item->setData(":/resources/standart.css");
    colors->addAction(item);
    color_group->addAction(item);

    colors->setTitle("Color Style");
    colors->setIcon(QIcon::fromTheme("preferences-desktop-color"));
